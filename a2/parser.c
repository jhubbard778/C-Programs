...
int main(int argc, char **argv) {
	...
	// Write cards output to cards.bin
	write_card();

	//       3. Sort the array
	qsort(cards, total_cards, sizeof(CARD_T *), card_name_comparator);
	
	// write card names and offset to index.bin
	write_index();
...
int dupe_check(unsigned id, char *name) {
	CARD_T **result = lfind(name, cards, &total_cards, sizeof(CARD_T *), cards_key_comparator);
	if (result) {
		if (id > (*result)->id) return -1;
...
void write_card() {
	for (int i = 0; i < total_cards; i++) {
		int text_size = strlen(cards[i]->text);

		indices = realloc(indices, sizeof(INDEX_T *) * (total_indices + 1));

		indices[total_indices] = malloc(sizeof(INDEX_T));
		indices[total_indices]->offset = ftello(cards_out);
		indices[total_indices]->card_name = cards[i]->name;
		
		total_indices++;

		// output to cards.bin
		fwrite(&cards[i]->id, 4, 1, cards_out);
		fwrite(&cards[i]->cost, 4, 1, cards_out);
		fwrite(&cards[i]->type, 4, 1, cards_out);
		fwrite(&cards[i]->class, 4, 1, cards_out);
		fwrite(&cards[i]->rarity, 4, 1, cards_out);
		fwrite(&text_size, 4, 1, cards_out);
		fwrite(cards[i]->text, text_size, 1, cards_out);
		fwrite(&cards[i]->attack, 4, 1, cards_out);
		fwrite(&cards[i]->health, 4, 1, cards_out);
	}
}

void write_index() {
	fwrite(&total_cards, 8, 1, index_out);
	// output to index.bin
	for (int i = 0; i < total_cards; i++) {

		// find card name in indices array to determine its index value in indices array for offset value
		INDEX_T **result = lfind(cards[i]->name, indices, &total_indices, sizeof(INDEX_T *), indices_key_comparator);
		if (!result) {
			printf("ERROR: No Name Found!\n");
			continue;
		}

		off_t offset = (*result)->offset;
		int name_size = strlen(cards[i]->name);
		
		fwrite(&name_size, 4, 1, index_out);
		fwrite(cards[i]->name, name_size, 1, index_out);
		fwrite(&offset, 8, 1, index_out);
	}
	
	// free the indices
	for (int i = 0; i < total_indices; i++) free(indices[i]);
	free(indices);
	return;
}